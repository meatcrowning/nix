...

{
  ...
  services.smartd = {
    ...
    devices = [
      ...
      { device = (builtins.elemAt privateConfig.smartDevices 0); }
      # cld — internal SATA HDD, btrfs data drive.
      { device = (builtins.elemAt privateConfig.smartDevices 1);
        options = "-a -n standby,q"; }
      # linux-old — internal SATA HDD, old generic-Linux root.
      { device = (builtins.elemAt privateConfig.smartDevices 2);
        options = "-a -n standby,q"; }
      # nixos-old — internal SATA SSD, previous NixOS install.
      { device = (builtins.elemAt privateConfig.smartDevices 3); }
    ];
  ...
  # Reserve the host-specific faulty memory regions recorded privately.
  # Removing these reservations can reintroduce silent data corruption.
  boot.kernelParams = privateConfig.badMemoryParams;
}
