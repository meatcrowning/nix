#!/usr/bin/env bash
# board-merge-test.sh — end-to-end test for the docs/board.md merge policy.
#
...
check "board.md carries merge=boardrecent" \
...
setup_repo
printf '%s' "$BASE" >board.md; git add -A
...
commit_at board.md "${BASE/bullet one/bullet one
- top added this}" 2000
git checkout -q book
commit_at board.md "${BASE/landed one/landed one
| abc123 | book added this |}" 3000
git checkout -q main
git fetch -q . book && git merge -q --no-edit FETCH_HEAD >/dev/null 2>&1
check "merge resolved" "$?" "0"
check "top's edit survived a NEWER book commit" "$(grep -c 'top added this' board.md)" "1"
check "book's edit survived" "$(grep -c 'book added this' board.md)" "1"
check "no conflict markers" "$(grep -c '^<<<<<<<\|^>>>>>>>' board.md)" "0"
...
GIT_AUTHOR_DATE="@1000 +0000" GIT_COMMITTER_DATE="@1000 +0000" git commit -q -m base
git branch -q book
commit_at board.md "${BASE/row one/row one - TOP VERSION}" 2000
git checkout -q book
commit_at board.md "${BASE/row one/row one - BOOK VERSION}" 3000
git checkout -q main
git fetch -q . book && git merge -q --no-edit FETCH_HEAD >/dev/null 2>&1
check "merge resolved" "$?" "0"
check "kept the newer (book) side" "$(grep -c 'BOOK VERSION' board.md)" "1"
check "dropped the older (top) side" "$(grep -c 'TOP VERSION' board.md)" "0"
check "no conflict markers" "$(grep -c '^<<<<<<<\|^>>>>>>>' board.md)" "0"
...
GIT_AUTHOR_DATE="@1000 +0000" GIT_COMMITTER_DATE="@1000 +0000" git commit -q -m base
git branch -q book
commit_at board.md "${BASE/row one/row one - TOP VERSION}" 4000
git checkout -q book
commit_at board.md "${BASE/row one/row one - BOOK VERSION}" 3000
git checkout -q main
git fetch -q . book && git merge -q --no-edit FETCH_HEAD >/dev/null 2>&1
check "merge resolved" "$?" "0"
check "kept the newer (top) side" "$(grep -c 'TOP VERSION' board.md)" "1"
check "dropped the older (book) side" "$(grep -c 'BOOK VERSION' board.md)" "0"

echo "== 4. the losing side is still recoverable from history =="
check "the dropped revision is reachable" \
  "$(git log --all -p -- board.md | grep -c 'BOOK VERSION')" "1"

echo "== 5. undatable sides fall back to union, never to a wedged sync =="
setup_repo
printf '%s' "$BASE" >board.md
...
"$DRIVER" board.md ours theirs 7 board.md
