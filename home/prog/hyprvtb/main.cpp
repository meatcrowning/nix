...
// are PySide, and neither can read a Hyprland config key. `hyprctl getoption`
// could answer, at the cost of a process spawn and a poll, and it would still
// only be a snapshot.
//
// So the plugin writes the resolved values out, exactly as vtbKinetic publishes
// its introspection: one small JSON file that a QML `FileView` and a
// `QFileSystemWatcher` both follow for free. Written on every config reload, so
// `hyprctl reload` after editing hyprland.lua retunes the ENTIRE desktop —
// compositor, panel and apps — with no restart of anything.
//
// TWO FILES, because the two readers can read different things and neither can
// read the other's:
//
//   motion.json    — the panel. Quickshell's `FileView` + `JSON.parse`, with
//                    watchChanges, so a reload lands live in a running panel.
//   DeskMotion.qml — the six apps (apps/qmlcommon/Motion.qml). Plain Qt QML has
//                    no file reader at all: `XMLHttpRequest` refuses a file://
//                    URL unless QML_XHR_ALLOW_FILE_READ is exported into every
//                    app (a blanket local-file-read permission granted to a
//                    WEB BROWSER among others — not a trade worth making for two
//                    integers), and there is no FileView outside Quickshell.
//                    A `Loader` pointed at an absolute file: URL needs no
//                    permission, and reports status Error for a missing file,
//                    which is exactly the fallback signal wanted. Verified both
//                    ways offscreen before it was written this way.
//
// Nothing untrusted reaches the generated QML: both values are numbers, already
// clamped by Cfg::, formatted by std::format. There is no path by which a
// config string becomes code here.
//
// Atomic (temp + rename): a reader must see the previous complete blob or the
// new one, never a half-written file. Failures are silent — a full disk must
// not take a code path the compositor depends on, and every reader carries its
// own default for exactly this case.
static void vtbPublishOne(const char* name, const std::string& body) {
    std::error_code ec;
    const auto      DIR = std::filesystem::path(vtbStateDir());
    std::filesystem::create_directories(DIR, ec);
    const auto FINAL = DIR / name;
...
void vtbPublishMotion() {
    if (!g_pGlobalState)
        return;

    const int   MS   = Cfg::slideDurationMs();
    const float FRAC = Cfg::rollSlideFrac();

    vtbPublishOne("motion.json",
                  std::format("{{\"slideMs\": {}, \"slideFrac\": {:.4f}, \"easing\": \"OutCubic\"}}\n", MS, FRAC));

    vtbPublishOne("DeskMotion.qml",
                  std::format("// GENERATED by hyprvtb (main.cpp, vtbPublishMotion) on every config\n"
                              "// reload. Do not edit: your changes are overwritten by the next\n"
                              "// `hyprctl reload`. The source of truth is plugin:hyprvtb:slide_duration_ms\n"
                              "// in hyprland.lua. Read by apps/qmlcommon/Motion.qml.\n"
                              "import QtQuick\n"
                              "QtObject {{\n"
                              "    readonly property int  slideMs:   {}\n"
                              "    readonly property real slideFrac: {:.4f}\n"
                              "}}\n",
                              MS, FRAC));
}
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    g_pGlobalState->config.textColor         = makeShared<Config::Values::CColorValue>("plugin:hyprvtb:col.text", "Title / glyph colour", 0xffffffff);
    ...
    // reference every sliding animation here is matched to (DESIGN.md §6.2),
    // so these are the numbers the panel's popups, the titlebar tooltip and the
    // apps' drawers all run at — published to ~/.local/state/hyprvtb/motion.json
    // by vtbPublishMotion() so those other two runtimes can actually read them
    // instead of hand-copying the constant, which is what they did until 2.89.
    //
    // The default is what the constants were: 260ms, 55% slide. Bounds are
    // declared so hyprlang REJECTS a bad value rather than the roll having to
    // survive one — 0ms is a division by zero in the progress step, and a frac
    // at either endpoint is a zero-width easing interval. Cfg:: clamps again.
    g_pGlobalState->config.slideDurationMs   = makeShared<Config::Values::CIntValue>(
        "plugin:hyprvtb:slide_duration_ms", "The desktop's canonical slide duration (ms) - window roll, panel popups, tooltips, app drawers", 260);
    ...
    // The outer-column title's orientation. Default false keeps the shipped
    // stacked column of upright letters; true lays the whole title sideways
    ...
    g_pGlobalState->config.titlebarEdge      = makeShared<Config::Values::CStringValue>(
        "plugin:hyprvtb:titlebar_edge", "Window edge the titlebar anchors to (right/left/top/bottom)", "right");
    ...
    // Publish the motion values now as well as on every reload. The panel and
    // the apps start independently of the compositor and may come up before the
    // first reload the listener above would see; without this they would run
    // their built-in defaults for the whole session on a machine whose
    // hyprland.lua had retuned them. (When the plugin is loaded during config
    // parse this writes the values seen SO FAR; the config.reloaded listener
    // fires at the end of that same parse and corrects it.)
    vtbPublishMotion();
    ...
    return {"hyprvtb", "Vertical per-window titlebars (close / roll-up / maximize / minimize / pin / program icon / stacked title) + app-button column via socket + KDE-style edge resize + MRU alt-tab + session save/restore + kinetic momentum scrolling", "lam", "3.50"};
}
