...
cleanup() {
  ...
  # Kill by CONFIG PATH. It cannot be $!: the compositor is spawned by
  # tools/sandbox.sh and is not this script's child. The path is unique per
  # run, so this can never touch the live session.
...
# process table, matched on the per-run config path. (The /proc/$HYPRPID/maps
# check near the end wants the real compositor pid, which is what this is.)
...
step "starting a nested Hyprland on vtb-a.so (off-screen via tools/sandbox.sh)"
