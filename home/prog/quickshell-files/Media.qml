...
Singleton {
    ...
    readonly property string dispTitle: px(hasPlayer ? (player.trackTitle || "")
        : useSnap ? (snap.title || "") : "")
    readonly property string dispArtist: px(hasPlayer ? (player.trackArtist || "")
        : useSnap ? (snap.artist || "") : "")
    ...
    readonly property var queue: {
        ...
        try {
            ...
            for (let i = 0; i < t.length; i++) {
                const r = t[i];
                const ti = root.px(r.title), ar = root.px(r.artist);
