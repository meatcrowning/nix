...
CREATE TABLE IF NOT EXISTS web_entity_matches (
  cache_key TEXT NOT NULL, entity_type TEXT NOT NULL,
  provider TEXT NOT NULL, entity_id TEXT, label TEXT,
  confidence REAL NOT NULL DEFAULT 0, status TEXT NOT NULL,
  candidates_json TEXT NOT NULL DEFAULT '[]', manual INTEGER NOT NULL DEFAULT 0,
  fetched_at REAL, error TEXT,
  PRIMARY KEY (cache_key, entity_type, provider)
);
...
CREATE TABLE IF NOT EXISTS web_metadata_overrides (
  cache_key TEXT NOT NULL, kind TEXT NOT NULL,
  body_json TEXT NOT NULL DEFAULT '{}', edited_at REAL NOT NULL,
  PRIMARY KEY (cache_key, kind)
);
...
def open_db():
    ...
    metadata_info = list(con.execute("PRAGMA table_info(web_metadata)"))
    metadata_pk = [r["name"] for r in sorted(metadata_info, key=lambda r: r["pk"]) if r["pk"]]
    if metadata_pk == ["cache_key", "kind", "source"]:
        con.execute("ALTER TABLE web_metadata RENAME TO web_metadata_old")
        con.execute("""CREATE TABLE web_metadata (
          cache_key TEXT NOT NULL, kind TEXT NOT NULL, source TEXT NOT NULL,
          body_json TEXT NOT NULL DEFAULT '{}', fetched_at REAL, expires_at REAL,
          error TEXT, PRIMARY KEY (cache_key, kind))""")
        con.execute("""INSERT OR REPLACE INTO web_metadata
          (cache_key,kind,source,body_json,fetched_at,expires_at,error)
          SELECT cache_key,kind,source,body_json,fetched_at,expires_at,error
          FROM web_metadata_old ORDER BY COALESCE(fetched_at, 0)""")
        con.execute("DROP TABLE web_metadata_old")
        con.commit()
    cols = {}
    added = False
...
class Library(QObject):
    ...
    def albums(self, sort="orig_year", descending=False):
        ...
        # release.  Keeping these as query-time aggregates avoids duplicating
        # mutable rating/play-count state in `albums` (and makes a click on a
        # star immediately affect the next sort).
        rows = self._rows("""
            SELECT a.*,
        ...
        if sort == "artist":
            ...
            rows.sort(key=lambda r: (r["added_at"] or 0,
                                     _artist_sortkey(r["album_artist"]),
                                     (r["album"] or "").casefold()))
        elif sort == "plays":
            rows.sort(key=lambda r: (r["play_count"] or 0,
                                     _artist_sortkey(r["album_artist"]),
                                     (r["album"] or "").casefold()))
        elif sort == "rating":
            rows.sort(key=lambda r: (-1 if r["rating"] is None else r["rating"],
                                     _artist_sortkey(r["album_artist"]),
                                     (r["album"] or "").casefold()))
        else:  # orig_year
...
class Player(QObject):
    ...
    def _sync_mpv(self, start_idx, paused=False, defer_rest=False):
        ...
        Long local tails fill in bounded event-loop batches on every sync,
        including manual skips. Remote queues retain their one-track lookahead.
        A new sync invalidates any callbacks for the old queue."""
        if not 0 <= start_idx < len(self._queue):
            return
        ...
        elif start_idx + 1 < len(self._queue) and (
                defer_rest or len(self._queue) - start_idx - 1 > self.RESTORE_APPEND_BATCH):
            self._mpv_fill_pending = True
            QTimer.singleShot(0, lambda: self._append_restored_tail(token, start_idx + 1))
        else:
            for row in self._queue[start_idx + 1:]:
    ...
    def queueTracks(self, ids):
        ...
        if self._mpv_fill_pending:
            # The fill reads the live queue length and will reach these rows.
            # Restarting the current file would interrupt an ordinary append.
            pass
        else:
            for r in fresh:
    ...
    def _resync_tail(self):
        ...
        try:
            ...
            self._mpv_loaded_until = self._index
            if library_is_remote_cached():
                self._ensure_mpv_lookahead(self._mpv_fill_token)
            elif len(self._queue) - self._index - 1 > self.RESTORE_APPEND_BATCH:
                token, next_idx = self._mpv_fill_token, self._index + 1
                self._mpv_fill_pending = True
                QTimer.singleShot(0, lambda: self._append_restored_tail(token, next_idx))
            else:
...
class Bridge(QObject):
    ...
    def __init__(self, library, player, lyrics, parent=None):
        ...
        self._player = player
        self._lyrics = lyrics
        self._now_info = NowPlayingMetadata(library, self)
        self._now_info.changed.connect(self._on_now_info)
        self._now_info_state = self._now_info.state
        ...
        player.currentChanged.connect(self._request_now_info)
    ...
    def _on_now_info(self, state):
        ...
        if state.get("trackId") and int(state["trackId"]) != int(current.get("id") or 0):
            return
    ...
    def _current_track(self):
        getter = getattr(self._player, "currentTrackDict", None)
        current = getter() if callable(getter) else getattr(self._player, "current", {})
        return current if isinstance(current, dict) else {}

    @Property("QVariantMap", notify=nowInfoChanged)
    def nowInfo(self):
        return dict(self._now_info_state)

    @Slot()
    def refreshNowInfo(self):
        current = self._current_track()
        self._now_info.refresh(int(current.get("id") or 0))

    @Slot()
    def clearNowInfo(self):
        current = self._current_track()
        self._now_info.clear(int(current.get("id") or 0))

    @Slot(str, result=bool)
    def chooseNowInfoMatch(self, entity_id):
        current = self._current_track()
        return self._now_info.choose(int(current.get("id") or 0), entity_id)

    @Slot(str, "QVariantMap", result=bool)
    def editNowInfo(self, kind, values):
        current = self._current_track()
        return self._now_info.edit(int(current.get("id") or 0), kind, dict(values))
    ...
    def setSort(self, sort):
        if sort in ("orig_year", "artist", "album", "added", "plays", "rating") and sort != self._sort:
            self._sort = sort
            self.refreshAlbums()

    @Slot(bool)
    def setSortDescending(self, descending):
...
def main():
    ...
    if plasma:
        ...
        # Qt 6.11's offscreen Oxygen plugin currently crashes inside
        ...
        # now-playing page is visible, publish a short-lived ownership lease:
        # the panel applet collapses and the larger transport copy appears.
        # Refreshing the lease makes a crashed player self-heal in the panel.
        runtime = Path(os.environ.get("XDG_RUNTIME_DIR")
                       or f"/run/user/{os.getuid()}")
        view_state = runtime / "player-view.json"
        ...
        publish_view()
        root.viewChanged.connect(publish_view)
        view_lease = QTimer(app)
        view_lease.setInterval(1000)
        view_lease.timeout.connect(publish_view)
        view_lease.start()

        def clear_view_state():
            try:
                state = json.loads(view_state.read_text())
                if int(state.get("pid", -1)) == os.getpid():
                    view_state.unlink()
            except (OSError, ValueError, TypeError):
                pass

        app.aboutToQuit.connect(clear_view_state)
