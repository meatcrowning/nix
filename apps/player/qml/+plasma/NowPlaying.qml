...
Item {
    ...
    readonly property int topH: Math.max(180, Math.min(height * 0.48, width * 0.58))
    readonly property int artW: Math.min(topH, Math.max(150, width * 0.39))
    ...
    Rectangle {
        ...
        gradient: Gradient {
            GradientStop { position: 0; color: root.lighter(Theme.bgAlt, 0.13) }
    ...
    Item {
        id: upperRight
        anchors { left: artBox.right; right: parent.right; top: parent.top }
        height: root.topH
