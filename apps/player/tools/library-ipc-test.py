...
FILES = []
rows = [
    ("Roygbiv", "Boards of Canada", "Music Has the Right to Children", 1, 1998, 5, 1, 12),
    ("Olson", "Boards of Canada", "Music Has the Right to Children", 2, 1998, 4, 0, 3),
    ("Xtal", "Aphex Twin", "Selected Ambient Works 85-92", 1, 1992, 5, 1, 40),
    ("Stone Age", "Machinedrum", "Psyconia", 2, 2021, None, 0, 0),
]
for i, (title, artist, album, tno, year, rating, fav, plays) in enumerate(rows, 1):
    f = TMP / ("%02d %s.flac" % (tno, title))
    f.write_bytes(b"not really audio")
    FILES.append(str(f))
    con.execute("INSERT INTO tracks VALUES (?,?,?,?,?,?,?,1,?,?,?,?,?,?,?,?)",
                (i, str(f), title, artist, album, artist, tno, year, "electronic",
...
check("an album comes back in play order",
      ...
      json.dumps(r)[:200])
r = call({"op": "stats"})
check("stats size the library", r.get("library", {}).get("tracks") == 4,
      json.dumps(r)[:160])
