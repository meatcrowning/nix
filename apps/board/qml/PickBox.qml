...
Item {
    ...
    //: the line the status footer shows while the pointer is on it. It says what
    ...
    signal hoveringChanged(bool on)
    ...
    Rectangle {
        anchors.fill: parent
        radius: 3
        color: ma.containsMouse ? Theme.highlight : "transparent"
        border.width: 1
    ...
    PixelText {
        ...
        // vocabulary.
        text: pick.label + "  v"
        color: ma.containsMouse ? pick.fgAccent : pick.fgDim
    }
    MouseArea {
        id: ma
        anchors.fill: parent
    ...
    HoverHandler {
        onHoveredChanged: pick.hoveringChanged(hovered)
    }
}
