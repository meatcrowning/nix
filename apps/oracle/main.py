...
MAKE_IMAGE_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "plus fetch_image.\n\n"
            "MANDATORY GATE — you must pass it on EVERY call, no exceptions: "
            "before you write a single word of the prompt, call "
            "use_skill() on the prompt skill that matches the model family "
            "(routing below) and write the prompt the way THAT skill says to. "
            "This is not optional, and none of these is a reason to skip it: "
            "\"I know how to prompt this\", \"I already loaded the skill "
            "earlier\", \"the skill's output was dropped to save context\", "
            "\"he just wants a quick tweak\". Call the skill fresh each time so "
            "its instructions are actually in front of you. A prompt written "
            "from memory instead of through the skill is a prompt he did not "
            "ask for.\n"
            "Routing — model → skill:\n"
            "  anima (the anime model) → use_skill('anima-prompt'). Then run "
            "your finished positive through booru_tags (the `check:` field) — "
            "a tag Danbooru does not have does nothing in the picture, and a "
            "prompt full of invented tags is half the image it looks like. "
            "Write tags with spaces, an artist as @name, name a CHARACTER by "
            "its tag plus its series tag (never hair/eyes from memory), and "
            "never open with 'masterpiece, best quality' or a score_ tag. "
            "Write your own `negative`; on anima it folds into the positive "
            "as an inline negative weight with its own box left empty.\n"
            "  krea / krea2 (the photoreal model) → use_skill('krea-prompt').\n"
            "  klein / Flux2-Klein (editing) → use_skill('flux-klein-edit').\n"
            "  any other family (chroma, z_image, qwen, sdxl, lumina, ...) → "
            "use_skill('krea-prompt') and follow its craft — a natural-language "
            "positive/negative pair with the medium and light named — unless a "
            "more specific prompt skill exists for it.\n"
            "Pass his own tags through VERBATIM — never rewrite a danbooru-"
            "style tag list. Give input_images to EDIT those pictures instead "
            "of generating a fresh one (that switches to the edit model on its "
            "own). PASS ONLY WHAT "
...
MAKE_VIDEO_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "neither for text-to-video. MANDATORY GATE — pass it on every call: "
            "before you write the prompt, call use_skill('video-prompt') and "
            "write it the way that skill says to. This is not optional and "
            "\"I already know how to prompt this\" is not a reason to skip it. "
            "Write the prompt as motion: what moves, "
...
def sampler_for(model, preset="default"):
    ...
    Qwen 3.6 always uses its published general-task thinking profile. For other
    models the family default is the floor's starting point: a
...
class Ollama(QObject):
    ...
    def _agent_post(self, run):
        ...
        with nothing at all."""
        # Subagents use the same model defaults as the main agent.
